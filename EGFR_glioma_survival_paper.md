...
Because CGGA EGFR expression does not track the biology EGFR is known to follow—reproducibly, across two batches, with verified data handling—the apparent "IDH-independent EGFR effect" cannot be interpreted as genuine, and CGGA does not provide a valid external test of the TCGA finding for this gene. A naive reading—reporting an IDH-independent, treatment-robust EGFR effect—would have been a spurious positive that only the positive controls exposed. The mechanistic cause of CGGA's attenuated EGFR signal remains unidentified and would require reprocessing from raw reads to resolve. Possible explanations include (i) differences in pre-analytical sample handling affecting EGFR RNA stability, (ii) batch effects in library preparation not fully corrected by CGGA's normalization pipeline, or (iii) mapping artifacts if EGFR-amplified regions create multi-mapping reads that CGGA's alignment workflow discards. The consistency across two independent RNA-seq batches (mRNAseq_693 and mRNAseq_325) suggests a systematic processing choice rather than random technical noise.
...
**Figure 6.** Positive control: standardized EGFR expression by WHO grade. EGFR rises with grade in TCGA (Spearman $r=+0.18$, $p<10^{-6}$; canonical biology) but is flat in CGGA (Spearman $r=-0.03$), indicating that CGGA EGFR expression does not reflect expected EGFR biology. Error bars represent mean ± standard error. The between-cohort difference is statistically significant (Fisher $r$-to-$z$ $p=2.4\times10^{-4}$). TCGA $n=826$ (grade II: 239, III: 259, IV: 328); CGGA $n=422$ (grade II: 111, III: 178, IV: 133).
...
For researchers conducting cross-cohort transcriptomic prognostic studies, we recommend a simple workflow: (1) identify a biological covariate known to associate with the gene of interest (e.g., tumor grade for oncogenes, hormone receptor status for breast cancer genes, molecular subtype for pathway members); (2) verify this association reproduces with consistent sign and magnitude in both the discovery and replication cohorts before interpreting cross-cohort prognostic comparisons; and (3) flag genes showing measurement discordance (expression–covariate correlations differing by Fisher $r$-to-$z$ $p<0.05$, or opposite signs, or $|r|<0.1$ in either cohort) for manual review or exclusion from cross-cohort inference. This gate is agnostic to gene identity, platform, and organism, requires only a single known covariate, and adds minimal computational cost while preventing a large fraction of measurement-artifact-driven false discordances.
...
All analyses were performed in R version 4.6.0 (2026-04-24) on macOS Tahoe 26.5.1 (Darwin 25.5.1, aarch64-apple-darwin24.6.0). Key package versions:

- **TCGAbiolinks** 2.40.0 (Bioconductor)
- **SummarizedExperiment** 1.42.0 (Bioconductor)
- **DESeq2** 1.52.0 (Bioconductor; for VST transformation)
- **survival** 3.8-6 (CRAN; Cox models, Kaplan-Meier)
- **survminer** 0.5.2 (CRAN; survival plotting)
- **dplyr** 1.1.4 (CRAN; data manipulation)
- **ggplot2** 3.5.1 (CRAN; visualization)

Matrix operations used OpenBLAS 0.3.33 (BLAS) and R's internal LAPACK 3.12.1. Full session information is archived with the analysis scripts at https://github.com/edanmn/TCGA-EGFR-Glioma.
