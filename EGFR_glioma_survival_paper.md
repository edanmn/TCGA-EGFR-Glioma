...
**Reproducibility.** The pipeline is deterministic except for the resampling analyses, which are seeded. All thresholds ($|r|>0.1$; CN $\geq6$) were pre-specified from biology. Analysis scripts (steps 00--39), console outputs and session information are at https://github.com/edanmn/TCGA-EGFR-Glioma. Four verification suites share no code with the analysis pipeline and reconstruct every reported sample size, refit every headline model, and assert every figure transcribed into the manuscript against its source output; `run_all.R` stops non-zero if any fails.
